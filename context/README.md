...
> AI dùng folder này để hiểu codebase. Có 2 hệ thống:
...
```bash
# Structural map (compressed — tiết kiệm token)
./scripts/generate-map.sh

# Full codebase export (cho web AI: ChatGPT, Claude Web)
./scripts/generate-context.sh
# hoặc: npx repomix
```

### Output files (đều GITIGNORED):
- `context/repo-map.md` — Khung xương code (fallback map)
- `context/repomix-output.md` — Full codebase snapshot

---

## Config
- `.mcp.json` — MCP server config cho code-review-graph
- `repomix.config.json` — Config Repomix ở root project
