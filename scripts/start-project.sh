...
while IFS= read -r line; do
  if [ -z "$line" ]; then
    ...
    # First empty line after content = break, first empty line at start = skip
    if [ -n "$BRIEF" ]; then
      break
    else
      break
...
# ── Step 3: AI tools selection ──────────────────────────────
echo ""
echo -e "${CYAN}Step 3/4: Chọn AI tools sử dụng${RESET}"
...
# ── Step 4: Setup files ─────────────────────────────────────
echo ""
echo -e "${CYAN}Step 4/4: Đang setup files...${RESET}"
...
# Write brain dump to docs/BRIEF.md
if [[ "$OSTYPE" == "darwin"* ]]; then
  sed -i '' "s|\[BRIEF_CONTENT\]|${BRIEF}|" docs/BRIEF.md 2>/dev/null || true
else
  sed -i "s|\[BRIEF_CONTENT\]|${BRIEF}|" docs/BRIEF.md 2>/dev/null || true
fi
...
elif [ -f "./scripts/generate-map.sh" ]; then
  ./scripts/generate-map.sh > /dev/null 2>&1 &
fi
EOF
